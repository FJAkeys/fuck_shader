...
Shader "Keys/toy" 
{
	Properties {
		...
		_ShadowWeight ("Shadow Weight", Float) = 0.6
	}
	SubShader {
		Tags {
		...
		Pass {
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
				...
				float3 WS_N = normalize(IN.normalWS);
				float3 WS_V = normalize(_WorldSpaceCameraPos - IN.positionWS);
				...
				// Diffuse
				half3 shading = (bakedGI + LightingLambert(attenuatedLightColor, WS_L, WS_N)) * _ShadowWeight + (1.0 - _ShadowWeight);

				//Fresnel
				float fresnel = pow((1.0 - saturate(dot(normalize(WS_N), normalize(WS_V)))), _FresnelPow);

				//TODO:
				// 1.if fresnel is effacting by shadow, make it an option.
				// 2.fresnel has parameter as smooth, threshold and intensity. (see URP stylized lit shader)
				// 3.fresnel color is somethin less needed, make it white.
				// 4.what is deractional fresnel?
				
				half4 color = baseMap * _BaseColor * IN.color + fresnel * _FresnelColor;

				half4 final = half4(shading * color.rgb , color.a);
