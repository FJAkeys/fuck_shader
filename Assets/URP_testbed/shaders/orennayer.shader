		...
		Pass {
			...
			half4 LitPassFragment(Varyings IN) : SV_Target {
				...
				/////////////////////////////////////////////////////////////////////////////////////////////
     			...
     			float ndotl = saturate(dot(WS_N, WS_L));
     			float ndotv = saturate(dot(WS_N, WS_V));
     			float oren_nayar_s = saturate(dot(WS_L, WS_V)) - ndotl * ndotv;
     			oren_nayar_s /= lerp(max(ndotl, ndotv), 1, step(oren_nayar_s, 0));

     			//lighting and final diffuse
     			float3 lightingModel = ndotl * (oren_nayar.x + color * oren_nayar.y + oren_nayar.z * oren_nayar_s);


				float3 shading = bakedGI + lightingModel * attenuatedLightColor;
     			
				float4 final =float4(shading * color.rgb, color.a);
